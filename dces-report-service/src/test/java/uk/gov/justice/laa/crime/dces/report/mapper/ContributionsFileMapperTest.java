...
class ContributionsFileMapperTest {
    ...
    void testProcessMultipleRequestFileGeneration() {
        ...
        try {
            ...
            String title = String.format("Monthly Contributions Report REPORTING DATE FROM: %s | REPORTING DATE TO: %s | REPORTING PRODUCED ON: %s\n", startDate, endDate, LocalDate.now());
            softly.assertThat(csvOutput).isEqualTo(title +
                    "MAAT ID,Data Feed Type,Assessment Date,CC OutCome Date,Correspondence Sent Date,Rep Order Status Date,Hardship Review Date,Passported Date,Transmission Date\n" +
